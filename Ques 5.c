...
// Function to match production S -> 0S0
bool match_S(char str[], int index, int length) {
  if (index >= length) {
  ...
  if (is_valid(str, index, '0')) {
    if (index + 2 < length && is_valid(str, index + 2, '0')) {
      return match_S(str, index + 1, length) && match_S(str, index + 3, length); // Check for 0S0
    } else {
      return false; // Invalid 0 without S0
    }
  } else {
